import type { KgrExcelRow, ExcelAlignedPhases, FsdMappingEntry } from "./types";
...
export function buildTraceabilityComments(row: KgrExcelRow, fsd: FsdMappingEntry): string {
  const numbered = parseNumberedSteps(row.testSteps);
  const expectedBullets = splitExpectedBullets(row.expectedResult);
  const describeLabel = row.subModule.replace(/^KYC Gap Report\s*-?\s*/i, "").trim() || "Core";

  return [
    `// Excel Test Case ID: ${row.id}`,
    `// Excel Scenario: ${escapeComment(describeLabel)} → ${escapeComment(row.taskDescription)}`,
...
export function buildInstrumentedTestBody(
  ...
  const label = row.subModule.replace(/^KYC Gap Report\s*-?\s*/i, "").trim() || "Core";

  const todoLine = todo ? `\n    ${todo}` : "";

  return `${traceability}${todoLine}
    console.log("[${row.id}] ${escapeComment(label)} → ${escapeComment(row.taskDescription)}");
    ${body}`;
}
