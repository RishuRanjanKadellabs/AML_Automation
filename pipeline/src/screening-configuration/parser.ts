...
export const SC_EXCEL_PATH = path.join(PROJECT_ROOT, "pipeline/test-data/Screening Configuration Test Cases.xlsx");
...
export function isScreeningConfigCaseId(id: string): boolean {
  return /^SC-TC-\d+$/i.test(id);
}

export function loadScRows(): ScExcelRow[] {
  const wb = XLSX.readFile(SC_EXCEL_PATH);
  const sheet = wb.Sheets[wb.SheetNames[0]];
  const raw = XLSX.utils.sheet_to_json<Record<string, string>>(sheet, { defval: "" });
  const seen = new Set<string>();

  return raw
    .filter((r) => isScreeningConfigCaseId(cellString(r["Test Case ID"])))
    .map((r) => ({
