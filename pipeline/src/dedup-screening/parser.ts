...
export const DDS_EXCEL_PATH = path.join(PROJECT_ROOT, "pipeline/test-data/Dedup Screening Test Cases.xlsx");
...
export function isDedupScreeningCaseId(id: string): boolean {
...
export function loadDdsRows(): DdsExcelRow[] {
  ...
  return raw
    .filter((r) => isDedupScreeningCaseId(cellString(r["Test Case ID"])))
    .map((r) => ({
...
export function describeLabel(subModule: string): string {
  return subModule
    .replace(/^De-Dup Screening\s*[–-]\s*/i, "")
    .trim() || "Core";
}
