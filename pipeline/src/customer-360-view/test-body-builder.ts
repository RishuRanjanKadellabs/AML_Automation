import type { C360ExcelRow, ExcelAlignedPhases, FsdMappingEntry } from "./types";
...
export function buildTraceabilityComments(row: C360ExcelRow, fsd: FsdMappingEntry): string {
...
export function buildInstrumentedTestBody(
  ...
  return `${traceability}${todoLine}
    console.log("[${row.id}] ${escapeComment(row.subModule)} → ${escapeComment(row.taskDescription)}");
    ${body}`;
}
