...
import { RDR_EXCEL_PATH } from "./parser";
...
function findIdColumn(headerMap: Map<ColumnKey, number>): number {
  ...
  if (!idCol) {
    throw new Error("Test Case ID column not found in workbook.");
  }
  return idCol;
}
...
function rowToValues(row: EnhancedRdrRow): Record<ColumnKey, string> {
  return {
    "Test Case ID": row.id,
    Module: row.module,
...
export async function writeEnhancedExcel(
  ...
  const headerMap = findHeaderMap(sheet.getRow(1));
  const idCol = findIdColumn(headerMap);
  const colCount = sheet.columnCount;

  const byId = new Map(rows.map((row) => [row.id, row]));
  const yellowIds = new Set([...highlightIds, ...rows.filter((r) => r.isNew).map((r) => r.id)]);
  let updatedCount = 0;

  sheet.eachRow((row, rowNumber) => {
    if (rowNumber === 1) return;
    const id = String(row.getCell(idCol).value ?? "").trim().toUpperCase();
    if (!id.startsWith("RDR_")) return;

    const enhanced = byId.get(id);
    if (!enhanced || enhanced.isNew) return;

    const values = rowToValues(enhanced);
    for (const [key, col] of headerMap.entries()) {
      if (key === "Test Case ID" || key === "Module" || key === "Submodule" || key === "Priority") {
        continue;
      }
